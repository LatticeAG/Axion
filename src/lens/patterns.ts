...
export const BELIEF_PATTERNS: BeliefPattern[] = [
  ...
  // ── Causal claims ──────────────────────────────────────────────────────
  // "because X" / "because of X" / ", because X"
  {
    label: 'because',
    type: 'causal',
    pattern: /\bbecause of\b\s*([^.;!?\n]{2,120}?)[.;!?\n]|because\s+([^.;!?\n]{2,120}?)[.;!?\n]/i,
    group: 2,
    confidence: 0.85,
  },
 ...
 * match and nudge the baseline confidence up or down.
 *
 * Each marker has a target band; the engine interpolates toward it. Order is
 * by descending strength so the strongest marker in a clause wins.
 */
export interface ConfidenceMarkerPattern {
  pattern: RegExp;
  /** Target confidence band when this marker is present. */
  confidence: number;
  label: string;
}

export const CONFIDENCE_MARKERS: ConfidenceMarkerPattern[] = [
  { label: 'certain', confidence: 1.0, pattern: /\b(?:definitely|certainly|absolutely|without a doubt|clearly|obviously|guaranteed)\b/i },
  { label: 'likely', confidence: 0.85, pattern: /\b(?:probably|likely|most likely|almost certainly|highly likely|strongly)\b/i },
  { label: 'possible', confidence: 0.6, pattern: /\b(?:might|may|could be|could possibly|perhaps|possibly|seems to)\b/i },
  { label: 'uncertain', confidence: 0.35, pattern: /\b(?:not sure|unsure|unclear|uncertain|i think|i believe|i guess|roughly|around)\b/i },
];
