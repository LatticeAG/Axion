/**
 * Axion Lens - OpenAI chat completions adapter.
 *
 * Routes `POST /v1/chat/completions`, validates a non-empty `messages[]`, and
 * normalizes non-streaming responses via the shared content extractor.
 */

import { extractOpenAIAssistantText } from "../content";
...
export const openaiAdapter: ProviderAdapter = {
  ...
  extractAssistantText(rawBody: string): string {
    return extractOpenAIAssistantText(rawBody);
  },
};

// Re-export the content helper for callers that want it directly.
export { extractOpenAIAssistantText };
